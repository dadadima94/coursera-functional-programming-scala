...
class ListsSuite {
  ...
  @Test def `sum with negative numbers in the list`: Unit = {
    assert(sum(List(4,7,9,-3,0,-5,-23)) == -11)
  }

  @Test def `max with only negative numbers in the list`: Unit = {
    assertEquals(max(List(-4, -7, -9, -3, -5)), -3)
  }
  ...
  @Test def `sum is 0 if list is empty`: Unit = {
    assert(sum(List()) == 0)
  }
